...
| `Tabs`           | `wpc-builder-tabs` (custom control)                                                             | _(no setting is registered, see below)_                                                                                                  | nothing is ever persisted. Built internally by `AbstractSection` when a section overrides `tabs()`, not meant to be added to a `fields()` array directly; see [tabs.md](tabs.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

This package ships 24 concrete field classes; two of them (`Toggle`,
...
### `Tabs` registers no setting

Same reasoning as `Custom` above: a `Tabs` field's own control renders a
group of buttons, not an editable value, so registering a setting for it
would only expose the same `transaction_fail` hazard for nothing gained.
See [tabs.md](tabs.md) for how a section actually uses it.
